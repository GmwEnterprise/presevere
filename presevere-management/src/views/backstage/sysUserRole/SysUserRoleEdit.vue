<template>
  <div>
    <form class="edit-form" @submit="submitForm">
      ...
      <div class="form-group">
        <label for="email">关联用户</label>
        <input class="form-control" id="userId" v-model="data.userId" placeholder="关联用户" required />
      </div>
      <div class="form-group">
        <label for="email">关联角色</label>
        <input class="form-control" id="roleId" v-model="data.roleId" placeholder="关联角色" required />
      </div>
      <div class="form-group">
...
<script>
import sysUserRoleService from './sysUserRole.service.js'
export default {
  ...
  methods: {
    async queryData(key) {
      const response = await sysUserRoleService.queryByKey(key)
      if (response.data) {
    ...
    submitForm(e) {
      ...
      if (this.data.id) {
        // 修改
        promise = sysUserRoleService.modify(this.data)
        flag = '修改'
      } else {
        // 新增
        promise = sysUserRoleService.add(this.data)
        flag = '新增'
      }
      promise
        .then(() => {
