<template>
  <div id="table-list-vue">
    ...
    <query-form @submit="querySubmit">
      ...
      <form-control v-model="formParam.userId" label="关联用户" id="userId" />
      <form-control v-model="formParam.roleId" label="关联角色" id="roleId" />
    ...
    <router-link
      to="/sys/modules/sysUserRole/edit"
      class="btn btn-light"
...
export default {
    ...
    return {
      ...
      table: {
        ...
        columns: [
          ...
          { code: 'userId', name: '关联用户', type: 'number', show: true },
          { code: 'roleId', name: '关联角色', type: 'number', show: true },
  ...
  methods: {
    async initTable() {
      const response = await sysUserRoleService.queryPage({
    ...
    deleteRow(rowId) {
      this.$message({
        ...
        btnName: '删除',
        event: async close => {
          await sysUserRoleService.delByKey(rowId)
